...
class Transcuti_model
{
    ...
    {
        $query = "SELECT t.*, k.nama AS karyawan, c.nama_cuti AS cuti FROM transcuti AS t INNER JOIN karyawan AS k ON t.karyawan_id = k.id INNER JOIN cuti AS c ON t.cuti_id = c.id WHERE ";
        

        // $query = "SELECT * FROM getCutiKaryawan t WHERE ";
        if($_SESSION['user']['nama_jabatan'] == 'Leader') {
            $query .= "t.approval_1 = :data ";
        } elseif($_SESSION['user']['nama_jabatan'] == 'Supervisor') {
            $query .= "t.approval_1 = :data OR t.approval_2 = :data ";
        } elseif($_SESSION['user']['nama_jabatan'] == 'Factory Manager') {
            $query .= "t.approval_1 = :data OR t.approval_2 = :data OR t.approval_3 = :data ";
        } elseif($_SESSION['user']['nama_jabatan'] == 'Manager') {
            $query .= "t.approval_1 = :data OR t.approval_2 = :data OR t.approval_3 = :data OR t.approval_4 = :data ";
        } elseif($_SESSION['user']['nama_jabatan'] == 'Admin' && $_SESSION['user']['nama_dept'] == 'HRD') {
            $query .= "t.approval_1 = :data OR t.approval_2 = :data OR t.approval_3 = :data OR t.approval_4 = :data OR t.approval_5 = :data ";
        }
    ...
    {
        // Nilai (5) pada klausa where adalah ID dari Master Cuti Tahunan. Dibuat static karena belum dapat cara bagaimana dapatkan ID Master Cuti Tahunan secara dynamic
        $this->db->query("SELECT kid.karyawan_id AS id_karyawan, nm.nama AS nama_karyawan, SUM(IF(status = 7, cuti_out, 0)) AS cuti_out FROM transcuti AS kid INNER JOIN karyawan AS nm ON kid.karyawan_id = nm.id WHERE cuti_id = 5 GROUP BY karyawan_id");
        return $this->db->resultSet();
    }
